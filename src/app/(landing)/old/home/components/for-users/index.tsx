...
import PhoneMockup from '@/app/(landing)/old/components/svgs/phone-mockup';
    ...
    const updateProgress = () => {
      refVideos.current.forEach((video, index) => {
        if (video && refProgressBars.current[index]) {
          ...
          refProgressBars.current[index]!.style.strokeDasharray =
            String(progress);
        }
      });
  ...
  useEffect(() => {
    ...
    refVideos.current.forEach((video, index) => {
      ...
      if (index !== active) {
        ...
        tl.to(
          [refProgressBars.current[index], refVideos.current[index]],
          { opacity: 0 },
          0
        );
        tl.to([refVideos.current[active], refMockupContainer.current], {
          opacity: 1,
        });
        ...
        <div className={styles.columns}>
          <div className={styles.content}>
            ...
            <p className={styles.text}>
              Gateway’s Self Sovereign Identity allows individuals to control
              access to their information across the web.
            </p>
                ...
                {data.map((item, index) => (
                  <div
                    ...
                    className={joinClasses(
                      styles.control,
                      active === index ? styles['control--active'] : ''
                    )}
                    onClick={() => {
                      setActive(index);
